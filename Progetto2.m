...
% PEV data
parms.Pnom_pev = 3.3; % Rated charging power [kW]
parms.Epev = 15; % PEV battery capaciy [kWh]
...
Pi_0 = zeros(Tf,1);        % total prosumerload [kW]
Pi = zeros(Tf,1);        % total consumerload [kW]
P_ul = zeros(Tf,1);
...
for k=1:Tf
    ...
    % compute control 
    [Pek,hatPck,hat_Pchk,hat_Pdsck,Pi_0k,Ppev(k),Pcool(k),Ph(k),Pabp(k),abp_varsk] = compute_control_step(parms,pek,ck,Pi_tot_forecast_k,T_ex_forecast_k,P_PV_forecast_k,P_ul_forecast_k,Tk,SoCk,SoCpevk,UR_hvac_k,UR_pev_k,SoCpev_objk,UR_abp_k,abp_varsk);
    ...
    hat_Pchk = min(Ppv(k),hat_Pchk);   % Check carica batteria solo ed esclusivamente da rinnovabile
    
    
    % simulate real system
    Pi(k) = Pi_tot(k,3);
    P_ul(k)= P_u_loads(k,3);
    Pexk = Ppv(k)+ hat_Pdsck - hat_Pchk - Pcool(k) - Ph(k)-Ppev(k)- P_ul(k)-Pabp(k);
    

    % battery comp

    if battery_compensation && Pexk ~= Pek-Pi_0k % forecast error must be compensated
        Pbk =-(Pek-Pi_0k)+ Ppv(k) - Pcool(k) - Ph(k)-Ppev(k)- P_ul(k)-Pabp(k);
        Pbk = max(Pbk,temp_Pb_min); % Saturate battery
        Pbk = min(Pbk,temp_Pb_max);
        Pch(k) = max(0,Pbk);
        Pdsc(k) = -min(0,Pbk);
        Pexk = Ppv(k)+ Pdsc(k) - Pch(k) - Pcool(k) - Ph(k)-Ppev(k)- P_ul(k)-Pabp(k);
    else
        Pch(k) = hat_Pchk;
        Pdsc(k) = hat_Pdsck;
    end
...
grid on
box on
legend('HVAC Power Consumption','Battery Power Exchange','PV Generation','Power shared','PEV charging power','ABP Power Consumption','Imported power','Exported power')
xlim([0 Tf-1])
xlabel('Time [h]')
ylabel('Power [kW]')
...
%% MPC step
function [Pek,Pck,Pchk,Pdsck,Pi_0k,Ppevk,Pcoolk,Phk,Pabpk,abp_varsk] = compute_control_step(parms,pek,ck,Pi_tot_forecast_k,T_ex_forecast_k,P_PV_forecast_k,P_ul_forecast_k,T_k,SoC_k,SoCpev_k,UR_hvac_k,UR_pev_k,SoCpev_objk,UR_abp_k,abp_varsk1)
%% AMPL SETUP
...
ampl.read('Progetto2.mod') %read the ampl file (.mod)
